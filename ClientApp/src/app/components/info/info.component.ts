...
export class InfoComponent implements OnInit {
  ...
  @ViewChild("uploadCanvas") uploadCanvas;
  ...
  changeImage(): void {
    ...
    if (input['files'] && input['files'][0]) {
      ...
      reader.onload = (e) => {
        huhu['src'] = e.target.result.toString();
        ctx.clearRect(0, 0, canvas.width, canvas.height);
        ctx.font = '30px Arial';
        ctx.fillStyle = '#FFFFFF';
        ctx.fillText('Loading Image...', 30, 50);
        setTimeout(() => {
          canvas.width = huhu.width;
          canvas.height = huhu.height;
          ctx.drawImage(huhu, 0, 0, huhu.width, huhu.height);
          console.log(canvas.toDataURL());
          this.myParty.avatar = canvas.toDataURL();
          console.log(this.myParty)
          this.setParty(this.myParty);
          console.log(this.getParty());
        }, 1000);
      };
